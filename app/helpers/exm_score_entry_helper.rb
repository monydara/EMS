module ExmScoreEntryHelper
	...
	def self.get_dtail_edit id
		data  =SdnClassStudent.joins( " 
				left join exm_tc_score_details e on e.student_id = sdn_class_students.id ")
	...
	def self.getCondition params
		...
		condition = "hr_employees.id!=-1 AND exm_tc_score_masters.is_deleted=0 "
		...
		if !assessment_id.nil? && !assessment_id.empty?
		    condition += " AND exm_tc_score_masters.assessment_type_id=#{assessment_id}"
		end
		
		if !from_date.nil? && !to_date.nil?  &&  !from_date.empty? && !to_date.empty?             
		    condition += "  AND  (exm_tc_score_masters.score_date BETWEEN '#{from_date}' AND '#{to_date}')  "
		end
	...
	def self.getSelectField data
		result = data.select("hr_employees.name AS teacher,	    					
	    					 exm_tc_score_masters.score_date,
	    					 ...
	    					 exm_tc_assessment_types.name AS assessment,
		...
		result = result.order(:score_date => :desc )
		result
	end
end
